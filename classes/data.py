...
class data(object):
    ...
    def __init__(self,raw,pp,fe,postp,fs,filename="",test=False):
        ...
        for idx,(post,c) in enumerate(self._postp):
            self._filename+="post"+str(post._id)
        ...
        for idx,(fs,c) in enumerate(self._fs):
            self._filename+="fs"+str(fs._id)
            if not changes and self.load() == True:
                print("Loaded "+fs._name+" from disk")
                continue
            print("FS: {}".format(fs))
            start = time.time()
            result = fs.run(self.D,c)
            if result is not None:
                self.D = result
            print("Taken: {}".format(time.time() - start))
            self.save()
